...
public class DropperInventory extends FakeInventory {
    ...
    public DropperInventory(InventoryHolder ih) {
        super(InventoryType.DROPPER, ih);
    }
    ...
    private void setDropper(Player p, BlockVector3 bv3) {
        UpdateBlockPacket ubp = new UpdateBlockPacket();
        ubp.x = bv3.x; ubp.y = bv3.y; ubp.z = bv3.z;
        ubp.blockRuntimeId = GlobalBlockPalette.getOrCreateRuntimeId(Block.DROPPER, 0);
    ...
    private static byte[] getNBT(BlockVector3 pos, String name) {
        CompoundTag tag = (new CompoundTag()).putString("id", "Dropper").putInt("x", pos.x).putInt("y", pos.y).putInt("z", pos.z).putString("CustomName", (name == null) ? "Dropper" : name);
        try {
