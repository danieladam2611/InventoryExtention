...
public class HopperInventory extends FakeInventory {
    ...
    public HopperInventory(InventoryHolder ih) {
        super(InventoryType.HOPPER, ih);
    }
    ...
    private void setHopper(Player p, BlockVector3 bv3) {
        UpdateBlockPacket ubp = new UpdateBlockPacket();
        ubp.x = bv3.x; ubp.y = bv3.y; ubp.z = bv3.z;
        ubp.blockRuntimeId = GlobalBlockPalette.getOrCreateRuntimeId(Block.HOPPER_BLOCK, 0);
    ...
    private static byte[] getNBT(BlockVector3 pos, String name) {
        CompoundTag tag = (new CompoundTag()).putString("id", "Hopper").putInt("x", pos.x).putInt("y", pos.y).putInt("z", pos.z).putString("CustomName", (name == null) ? "Hopper" : name);
        try {
